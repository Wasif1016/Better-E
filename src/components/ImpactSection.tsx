  ...
  return (
    <section id="impact" className="py-16 md:py-24">
      <div className="max-w-[1400px] mx-auto px-4">
        ...
        <div className="flex flex-col items-center gap-4 mt-8 md:mt-12 px-8">
          <div className="text-center max-w-2xl mx-auto bext-base md:text-lg text-foreground/90">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 w-full max-w-3xl">
            ...
            <div className="flex flex-col items-center bg-foreground/[0.02] p-6 shadow-sm border  text-center">
              <div className="text-2xl md:text-3xl font-bold text-black mb-1">€80</div>
              <div className="font-medium text-gray-800 text-sm md:text-base">aan batterij kosten</div>
              <div className="text-foreground/70 text-sm">(per gebruiker per jaar)</div>
            </div>

            {/* 2. 0,84 kg aan kritieke grondstoffen */}
            <div className="flex flex-col items-center bg-foreground/[0.02] p-6 shadow-sm border  text-center">
              <div className="text-2xl md:text-3xl font-bold text-black mb-1">0,84 kg</div>
              <div className="font-medium text-gray-800 text-sm md:text-base">aan kritieke grondstoffen</div>
              <div className="text-foreground/70 text-sm">(per gebruiker per jaar)</div>
            </div>

            {/* 3. Significant risico op brandgevaar */}
            <div className="flex flex-col items-center bg-foreground/[0.02] p-6 shadow-sm border  text-center">
              <div className="text-2xl md:text-3xl font-bold text-black mb-1">Significant</div>
              <div className="font-medium text-gray-800 text-sm md:text-base">risico op brandgevaar</div>
              <div className="text-foreground/70 text-sm">(minder risico per jaar)</div>
            </div>
          </div>
        </div>

        {/* Impact Features with Accordion */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-4 md:mt-8 px-0 md:px-8">
          {impactFeatures.map((feature) => (
            <div key={feature.id} className="h-full bg-background hover:bg-foreground/[0.02] p-6 shadow-sm border  flex flex-col">
                ...
                <div className="mt-auto">
                  ...
                  {expandedSection === feature.id && (
                    <div className="mt-3 pt-3 border-t border-gray-200 animate-in slide-in-from-top-2 duration-200">
                      <p className="text-foreground/90 text-sm">{feature.content}</p>
                    </div>
                  )}
                </div>
              </div>
